...
## 📈작업자별 생산성 및 오류율 통합분석
### 🛠️사용 SQL
- [SQL : 작업자별 UHP·오류율·HTP 산정 쿼리](./SQL/GITHUB_생산성지표.sql)
...
> 
> 1. **생산성(UPH) 산정 기준**
>    * 생산성(UPH)은 로케이션 수가 아닌 **실제 상품 처리수량 기준으로 산정**. 동일 로케이션이라도 상품 수량 차이에 따라 실제 작업 강도가 달라질 수 있기 때문이다. (로케이션당 처리 상품수 (작업 범위 기준 생산성)
>    * UHP = 총 처리 상품수 / 총 로케이션 수 (로케이션당 처리량)
>
> 2.**생산성(HTP) 산정 기준**
>     * HTP = 총 처리 상품수 / 총 소요시간(분) (시간당 처리량)
>     * 시간당 처리 상품수 (작업 시간 기준 효율)
> 
> 3. **불일치율 지표 산정 기준**
>    * 본 쿼리의 불일치율 지표는 특정 로케이션의 최종 마감 상태를 보는 결과 중심 지표가 아니다.
>    * 작업자가 현장에서 수행한 **모든 SBC 조사 내역을 집계하여, 작업 과정 중 발생한 '불일치 판정'을 누적 계산**하였다.
>    * 이를 통해 로케이션 환경에 가려질 수 있는 **작업자 개인의 순수 품질 리스크(Human Error 성향)를 왜곡 없이 추출**하고자 하였다.
...
> 다만 센터 내 상대적 분포를 기준으로 보면, 품질리스크형 작업자의 평균 오류율은 센터 전체 평균(8.88%) 대비 약 1.5배 수준으로, 집중 관리가 필요한 그룹임을 확인할 수 있다. 해당 수치는 탐색적 분석 결과이며, 추후 기간 확장 및 다센터 비교를 통한 검증이 필요하다.
