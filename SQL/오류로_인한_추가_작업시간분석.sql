...
time_metrics AS (
    ...
    SELECT
        판정_상세유형,
        CASE
            WHEN 판정_상세유형 IN ('SBC오류', 'CC1차_오류', 'CC2차_오류', '복합오류')
                 AND `최종 작업시간` IS NOT NULL
...
SELECT
    ...
    ROUND(AVG(CASE WHEN 판정_상세유형 IN ('SBC오류', 'CC1차_오류', 'CC2차_오류', '복합오류')
                   THEN duration_sec END), 1) AS 오류건_평균소요시간,

    COUNT(CASE WHEN 판정_상세유형 IN ('SBC오류', 'CC1차_오류', 'CC2차_오류', '복합오류')
               THEN 1 END) AS 실오류_확정건수,

    ROUND(
        AVG(CASE WHEN 판정_상세유형 IN ('SBC오류', 'CC1차_오류', 'CC2차_오류', '복합오류')
                 THEN duration_sec END)
        - AVG(CASE WHEN 판정_상세유형 = '정상(종료)'
                   THEN duration_sec END),
        1
    ) AS 오류1건당_지연시간,
